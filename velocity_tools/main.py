...
# If you start at non-attractor states, time point 0 will not be reflective of the landscape and will skew the
# velocity calculation (via gamma fit), so we need to drop this timepoint for the fitting.
vlm.filter_cells(vlm.ca['SampleID'] != '0')
...
PCA(vlm, n_components=2, pick_cutoff=False, which_S='S')
...
################################################################################################################
'''5. Plotting data'''
# plot_2_gene_arrows(vlm, which_S="Sx", separate=True)
...
vlm.Sxt = vlm.Sx.T
estimate_transition_prob_smg(vlm, hidim = 'Sx', embed='Sxt', transform = 'sqrt', n_neighbors=20, sampled_fraction=1,
                             calculate_randomized = False)
vlm.calculate_embedding_shift(sigma_corr=0.05, expression_scaling=False)
vlm.calculate_grid_arrows(smooth = 0.8, steps =(40,40), n_neighbors=20)
print(vlm.transition_prob)
plot_arrows(vlm, type = 'embedded')
